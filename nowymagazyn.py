...
sb.table("products").insert({"sku": sku, "name": name, "unit": unit}).execute()
st.success("Dodano produkt.")

st.divider()
st.subheader("Lista produktów")
products = sb.table("products").select("id,sku,name,unit,created_at").order("created_at", desc=True).execute()
...
products = sb.table("products").select("id,sku,name,unit").order("name").execute()
...
moves = sb.table("stock_moves").select("created_at,move_type,qty,note,product_id").order("created_at", desc=True).limit(50).execute()
